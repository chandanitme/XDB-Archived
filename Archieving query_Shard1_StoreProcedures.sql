...
INTO   #temp
FROM   [sc10directaxis_Xdb.Collection.Shard1].[xdb_collection].[interactions]
...
WHILE EXISTS (SELECT 1
  ...
  BEGIN
      ...
      BEGIN try
          ...
          SELECT *
          FROM   [sc10directaxis_Xdb.Collection.Shard1].[xdb_collection].[interactionfacets]
          ...
          DELETE FROM [sc10directaxis_Xdb.Collection.Shard1].[xdb_collection].[interactionfacets]
          WHERE  interactionid IN (SELECT interactionid
                                   FROM   #temp
                                   WHERE  rownum > @ProcessedCount
                                          AND rownum <= ( @ProcessedCount +
          ...
          DELETE FROM [sc10directaxis_Xdb.Collection.Shard1].[xdb_collection].[interactions]
          WHERE  interactionid IN (SELECT interactionid
                                   FROM   #temp
                                   WHERE  rownum > @ProcessedCount
                                          AND rownum <= ( @ProcessedCount +
